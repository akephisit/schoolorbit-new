...
### ✅ M-7. Shared API contract ระหว่าง backend-school และ frontend — Activity workspace batch เสร็จแล้ว
...
| **ผลลัพธ์** | ปัจจุบัน contract มี 165 unique operations: 32 auth/authorization, 36 read-oriented JSON operations จาก checkpoint เดิม, Phase 1 people จำนวน 12 mutations พร้อม achievement list read, Phase 2 academic structure จำนวน 15 mutations พร้อม 4 dependent reads, Phase 2 curriculum core จำนวน 15 mutations พร้อม 9 dependent reads, Phase 2 activity-template จำนวน 10 mutations พร้อม 3 dependent reads และ Phase 2 activity workspace จำนวน 21 mutations พร้อม 7 dependent reads |
...
| **ความถูกต้อง/ความปลอดภัย** | เมื่อ `users.status != 'active'` effective permissions ต้องว่างและ student soft delete ส่ง `permission_changed`; Academic structure/classroom/enrollment handlers ใช้ `actor_tenant_context`; Subject/Study Plan, activity-template และ activity-workspace backend บังคับ permission/resource policy ตรงกับ frontend; missing parent/target คืน not-found, bounded role/result ผ่าน validation, bulk count คืนค่าที่เขียนจริง และ capacity-sensitive enrollment ทำใน transaction เดียวกัน |
| **งานต่อ Phase 4 mutation-contract rollout** | Phase 2 course planning, teaching assignments และ scheduling configuration เป็นชุดถัดไป; SSE, WebSocket, health/readiness และ file/binary endpoints ยังอยู่นอก OpenAPI contract โดยตั้งใจ |
| **ความยาก** | Medium |
