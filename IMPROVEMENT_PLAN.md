...
### ✅ M-7. Shared API contract ระหว่าง backend-school และ frontend — Activity template batch เสร็จแล้ว
...
| **ผลลัพธ์** | ปัจจุบัน contract มี 137 unique operations: 32 auth/authorization, 36 read-oriented JSON operations จาก checkpoint เดิม, Phase 1 people จำนวน 12 mutations พร้อม achievement list read, Phase 2 academic structure จำนวน 15 mutations พร้อม 4 dependent reads, Phase 2 curriculum core จำนวน 15 mutations พร้อม 9 dependent reads และ Phase 2 activity-template จำนวน 10 mutations พร้อม 3 dependent reads |
...
| **ความถูกต้อง/ความปลอดภัย** | เมื่อ `users.status != 'active'` effective permissions ต้องว่างและ student soft delete ส่ง `permission_changed`; Academic structure/classroom/enrollment handlers ใช้ `actor_tenant_context`; Subject/Study Plan และ activity-template backend บังคับ generated permission policy ตรงกับ frontend; activity generation ต้องมี `ACTIVITY_MANAGE_ALL` และ curriculum read; missing target คืน not-found, path version กำหนด query scope, bulk count คืนค่าที่ insert จริง และ update instructor ที่ไม่พบต้องไม่ demote primary เดิม |
| **งานต่อ Phase 4 mutation-contract rollout** | Phase 2 activity workspace (slots, groups, instructors, classroom assignments และ members) เป็นชุดถัดไป โดยยังต้องตรวจ behavior, permission, status และ response DTO ทีละกลุ่ม; SSE, WebSocket, health/readiness และ file/binary endpoints ยังอยู่นอก OpenAPI contract โดยตั้งใจ |
| **ความยาก** | Medium |
