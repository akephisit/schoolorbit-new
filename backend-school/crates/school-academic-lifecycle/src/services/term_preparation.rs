...
use super::super::models::{
    ...
    TermPreparationDateRequirement, TermPreparationMappingOption,
    TermPreparationMappingRequirement, TermPreparationMappings, TermPreparationModuleEvidence,
...
use school_academic_core::{
    ...
    services::{lifecycle_guard, years_terms},
};
use school_academic_delivery::services::offerings;
...
#[doc(hidden)]
pub struct NormalizedPreparationInput {
...
struct PreviewState {
    ...
    delivery_preview: Option<school_academic_delivery::models::CurriculumOfferingPreview>,
}

#[doc(hidden)]
pub fn validate_checksum(value: &str) -> Result<(), AppError> {
    if value.len() == 64
        && value
            .bytes()
...
fn prospective_delivery_options(
    target_term_id: Uuid,
    preview: Option<&school_academic_delivery::models::CurriculumOfferingPreview>,
...
pub async fn apply(
    ...
    for module in &normalized.modules {
        let outcome = match module {
            ...
            TermPreparationModule::Assessments => {
                school_academic_assessment::assessment::services::apply_term_preparation(
                    &mut tx,
                    actor.user_id,
...
async fn apply_timetable(
    ...
    school_academic_timetable::services::timetable_version_service::apply_term_preparation(
        tx,
        actor,
        context,
        &mappings.entities,
...
async fn apply_exams(
    ...
    external
        .apply_exam_preparation(tx, actor, context, &mappings.entities, &mappings.dates)
        .await
}
...
async fn apply_supervision(
    ...
    external
        .apply_supervision_preparation(tx, actor, context, &mappings.entities, &mappings.dates)
        .await
}
