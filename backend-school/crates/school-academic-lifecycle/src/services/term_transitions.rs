use super::super::models::{TermTransitionAction, TermTransitionOutcome, TermTransitionRequest};
use chrono::NaiveDate;
use school_academic_core::models::{AcademicTermStatus, AcademicYearStatus, TermLifecycleContext};
use school_academic_core::services::{lifecycle_context, lifecycle_guard, years_terms};
...
pub(crate) async fn read_context(
    ...
    lifecycle_context::read_term_context(tx, year, term).await
}
...
pub async fn transition_term(
    ...
    if request.action == TermTransitionAction::Activate {
        return super::term_activation::activate_term(pool, actor, term, request).await;
    }
    use crate::{models::LifecycleSeverity, services};
    ...
    let workspace = services::workspace_in_transaction(
        external,
        &mut tx,
        actor,
    ...
    if request.action == TermTransitionAction::Reopen {
        school_academic_results::services::require_term_without_annual_results(
            &mut tx,
            request.academic_year_id,
            term,
        )
