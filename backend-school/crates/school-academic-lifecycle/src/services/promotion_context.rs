...
use crate::models::{PromotionPolicyOptions, PromotionRuleInput, PromotionSuccessOutcome};
use school_academic_core::models::{GradeProgression, GradeProgressionKind, GradeProgressionSet};
...
pub(crate) async fn policy_options(
    ...
    let grades: Vec<crate::models::PromotionGradeReference> = sqlx::query_as(
...
pub(crate) async fn validate_promotion_rules(
    ...
    for rule in rules {
        ...
        let kind = success_progression_kind(rule.success_outcome);
...
fn success_progression_kind(outcome: PromotionSuccessOutcome) -> GradeProgressionKind {
    match outcome {
...
mod tests {
    ...
    #[test]
    fn success_outcomes_map_to_the_exact_core_progression_kind() {
        assert_eq!(
            success_progression_kind(PromotionSuccessOutcome::Promote),
            GradeProgressionKind::Promote
        );
        assert_eq!(
            success_progression_kind(PromotionSuccessOutcome::Graduate),
            GradeProgressionKind::Graduate
        );
    }
}
