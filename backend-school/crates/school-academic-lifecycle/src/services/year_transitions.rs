use super::super::models::{YearTransitionAction, YearTransitionOutcome, YearTransitionRequest};
use school_academic_core::models::{
    AcademicYearStatus, YearLifecycleContext, YearTermLifecycleState,
};
use school_academic_core::services::{
    lifecycle_context, lifecycle_guard, year_commands, years_terms,
};
...
pub(crate) async fn read_context(
    ...
    lifecycle_context::read_year_context(tx, year).await
}
...
pub(crate) async fn read_terms(
    ...
    lifecycle_context::read_year_terms(tx, year).await
}
