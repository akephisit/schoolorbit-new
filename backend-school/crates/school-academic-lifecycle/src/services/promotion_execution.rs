...
use school_academic_results as results;
...
#[doc(hidden)]
pub const RECEIPT_COLUMNS: &str = "item_id,run_id,request_id,approval_id,target_student_year_id,target_placement_id,source_row_version,executed_by,executed_at";
...
pub async fn execute_run(
    ...
    for item_id in &batch.item_ids {
        if let Err(error) = execute_item(pool, &batch, *item_id).await {
            ...
            let message = execution_failure_message(&error).to_owned();
...
fn execution_failure_message(error: &AppError) -> &str {
    match error {
...
#[doc(hidden)]
pub async fn execute_item(pool: &PgPool, batch: &Batch, item_id: Uuid) -> Result<(), AppError> {
    let mut tx = pool.begin().await?;
    lifecycle_guard::lock_transition(&mut tx).await?;
    ...
    let core_decision = decision.core_command();
