...
use school_academic_core::services::{lifecycle_guard, promotion_students, promotion_targets};
...
#[doc(hidden)]
pub fn intent_checksum(run: &PromotionRun, items: &[PromotionRunItem]) -> Result<String, AppError> {
...
pub async fn approve_run(
    ...
    for batch in pending.chunks(500) {
        ...
        for item in batch {
            ...
            destinations.push((source, decision.core_command()));
        }
        let destination_refs: Vec<_> = destinations
            .iter()
            .map(|(source, decision)| (*source, decision))
            .collect();
        promotion_targets::validate_destinations(&mut tx, run.target_year_id, &destination_refs)
            .await?;
    }
