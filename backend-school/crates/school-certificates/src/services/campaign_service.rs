...
use crate::{
    ...
    models::{
        ...
        CertificateCampaignStatus, CertificateCampaignSummary, CertificateOwnerOption,
...
pub async fn update_campaign(
...
) -> Result<CertificateCampaignDetail, super::request_service::CertificateServiceError> {
    ...
    if changed_fields.is_empty() {
        tx.commit().await.map_err(campaign_db_error)?;
        return Ok(fetch_detail_with_capabilities(pool, actor, campaign_id).await?);
    }
    ...
    Ok(fetch_detail_with_capabilities(pool, actor, campaign_id).await?)
}
...
pub async fn change_campaign_status(
    ...
    if payload.status == CertificateCampaignStatus::Purging {
        return Err(
            AppError::ValidationError("สถานะกำลังลบตั้งค่าได้เฉพาะกระบวนการลบถาวร".to_string()).into(),
        );
    }
    let authorization_row = fetch_campaign_row(pool, campaign_id).await?;
    require_owner_action(
...
pub async fn list_owner_options(
    pool: &PgPool,
    actor: &ActorContext,
) -> Result<Vec<CertificateOwnerOption>, AppError> {
    ...
    Ok(rows
        .into_iter()
        .map(|row| CertificateOwnerOption {
            id: row.id,
            code: row.code,
            name: row.name,
            name_en: row.name_en,
