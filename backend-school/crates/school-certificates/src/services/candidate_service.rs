...
pub async fn update_candidate(
...
) -> Result<CertificateCandidateDetail, super::request_service::CertificateServiceError> {
    ...
    if locked.updated_at != request.expected_updated_at {
        return Err(AppError::Conflict("รายชื่อผู้รับถูกแก้ไขจากหน้าจออื่น กรุณาโหลดใหม่".to_string()).into());
    }
    ...
    if previous_type != request.recipient_type && !had_invalid_type {
        return Err(
            AppError::Conflict("การเปลี่ยนเป็นบุคคลภายนอกต้องใช้คำสั่งยืนยันเฉพาะ".to_string()).into(),
        );
    }
    ...
    .await?;
    tx.commit().await.map_err(candidate_db_error)?;
    Ok(get_candidate(pool, actor, candidate_id).await?)
}
...
) -> Result<CertificateCandidateBulkResult, super::request_service::CertificateServiceError> {
...
async fn bulk_update_inner(
    ...
    if candidate_ids.is_empty() || candidate_ids.len() > MAX_BULK_CANDIDATES {
        return Err(AppError::ValidationError("ต้องเลือกรายชื่อผู้รับ 1–5,000 รายการ".to_string()).into());
    }
    if candidate_ids.len() != request.candidate_ids().len() {
        return Err(AppError::ValidationError("รายการที่เลือกมีรหัสซ้ำ".to_string()).into());
    }
    ...
    for (row, input) in rows.iter().zip(import_rows.iter()) {
        ...
        match &request {
            ...
            CertificateCandidateBulkRequest::ChooseName { name_source, .. } => {
                ...
                {
                    return Err(
                        AppError::Conflict("บางรายการไม่มีชื่อบัญชีและชื่อไฟล์ให้เลือก".to_string()).into(),
                    );
                }
            ...
            CertificateCandidateBulkRequest::ConfirmDuplicate { .. } => {
                ...
                {
                    return Err(
                        AppError::Conflict("บางรายการไม่มีคำเตือนรายการซ้ำให้ยืนยัน".to_string()).into(),
                    );
                }
...
fn require_candidate_mutable(
    ...
    if candidate.issued_certificate_id.is_some() {
        return Err(AppError::Conflict("รายชื่อที่ออกเกียรติบัตรแล้วแก้ไขไม่ได้".to_string()).into());
    }
