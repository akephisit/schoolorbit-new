...
pub enum CertificateServiceError {
    ...
    #[error("certificate resource is locked")]
...
pub async fn submit_issue_request(
...
) -> Result<CertificateIssueRequestDetail, CertificateServiceError> {
    ...
    {
        return Err(AppError::NotFound("ไม่พบรายชื่อที่เลือกในกิจกรรมนี้".to_string()).into());
    }
    ...
    Ok(fetch_request_detail_unchecked(pool, actor, request_id).await?)
}
...
pub fn resource_locked_error(request_id: Uuid, can_read_request: bool) -> CertificateServiceError {
    CertificateServiceError::ResourceLocked {
