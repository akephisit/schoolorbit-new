...
use school_file_platform::platform_types::{FileInspectionMetadata, PdfPageBox};
use school_fonts::{
    models::{
...
use crate::{
    access_policy::{require_owner_action, require_template_action, CertificateAction},
    models::{
        ...
        CreateCertificateTemplateRequest, GeometryAction, PageGeometry, RecipientType,
...
pub async fn update_template(
...
) -> Result<CertificateTemplateMutationOutcome, super::request_service::CertificateServiceError> {
    ...
    if locked.updated_at != payload.expected_updated_at {
        return Err(AppError::Conflict("แม่แบบถูกแก้ไขแล้ว กรุณาโหลดข้อมูลล่าสุด".to_string()).into());
    }
    ...
    let custom_variables = if let Some(page) = page {
        ...
        return Err(AppError::Conflict("ต้องแนบพื้นหลังที่ถูกต้องก่อนบันทึกองค์ประกอบ".to_string()).into());
    } else {
        Vec::new()
    };
...
pub async fn attach_background(
    ...
    let FileInspectionMetadata::Pdf {
        ...
        return Err(AppError::ValidationError("พื้นหลังต้องเป็น PDF ที่ตรวจสอบแล้ว".to_string()).into());
    };
    if page_count != 1 {
        return Err(AppError::ValidationError("พื้นหลังเกียรติบัตรต้องมีหนึ่งหน้า".to_string()).into());
    }
    ...
    if let Some(old_page) = source_page_geometry(&locked)? {
        ...
        if geometry_changed && !payload.preview_confirmed {
            return Err(
                AppError::Conflict("ต้องพรีวิวและยืนยันการจัดวางเมื่อขนาดพื้นหลังเปลี่ยน".to_string()).into(),
            );
        }
...
pub async fn attach_asset(
...
) -> Result<CertificateTemplateDetail, super::request_service::CertificateServiceError> {
    let authorization = fetch_template_row(pool, template_id).await?;
    require_owner_action(
        pool,
        actor,
    ...
    if !matches!(
        ...
        return Err(AppError::ValidationError("ชนิดไฟล์ไม่ตรงกับชนิดทรัพยากรแม่แบบ".to_string()).into());
    }
    ...
    .await?;
    tx.commit().await.map_err(template_db_error)?;
    Ok(fetch_detail_with_capabilities(pool, actor, template_id).await?)
}
...
pub async fn delete_template(
    ...
    actor: &ActorContext,
    template_id: Uuid,
) -> Result<CertificateTemplateDeleteOutcome, super::request_service::CertificateServiceError> {
    let authorization = fetch_template_row(pool, template_id).await?;
    require_owner_action(
        pool,
        actor,
    ...
    if has_candidate {
        return Err(
            AppError::Conflict("ยังมีรายชื่อผู้รับอ้างถึงแม่แบบนี้ กรุณาย้ายหรือลบรายชื่อก่อน".to_string()).into(),
        );
    }
...
async fn require_locked_campaign_owner_unchanged(
    ...
    campaign_id: Uuid,
) -> Result<(), super::request_service::CertificateServiceError> {
    let locked_owner_id = lock_campaign_owner(tx, campaign_id).await?;
    Ok(require_authorized_owner_unchanged(
        authorized_owner_id,
        locked_owner_id,
    )?)
}
