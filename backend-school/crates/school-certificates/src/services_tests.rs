...
use crate::{
    access_policy::{require_owner_action, CertificateAction},
    models::{
        ...
        CertificateAccountSearchQuery, CertificateCampaignListQuery,
        ...
        CertificateFontSource, CertificateImportRequest, CertificateImportRowInput,
        ...
        ManualCertificateVerificationRequest, NullableUuidUpdate, QrCertificateVerificationRequest,
        RecipientType, RevokeCertificateRequest, StartCertificateCampaignPurgeRequest,
...
use request_service::CertificateServiceError;
...
#[async_trait]
impl school_file_platform::storage_provider::StorageProvider for PreviewStorage {
    async fn check_readiness(
        &self,
    ) -> Result<(), school_file_platform::storage_provider::StorageError> {
        Ok(())
    }

    async fn put(
        &self,
        _object: &school_file_platform::storage_provider::StoredObject,
        _body: Bytes,
    ) -> Result<(), school_file_platform::storage_provider::StorageError> {
        Ok(())
    }
    ...
    async fn get(
        ...
        _object: &school_file_platform::storage_provider::StoredObject,
        _max_bytes: u64,
    ) -> Result<Bytes, school_file_platform::storage_provider::StorageError> {
        Ok(Bytes::new())
    }
    ...
    async fn head(
        ...
        Option<school_file_platform::storage_provider::ObjectMetadata>,
        school_file_platform::storage_provider::StorageError,
    > {
        Ok(None)
    }
    ...
    async fn private_download_grant(
        ...
        school_file_platform::platform_types::DownloadGrant,
        school_file_platform::storage_provider::StorageError,
    > {
        Ok(
            school_file_platform::platform_types::DownloadGrant::Redirect {
    ...
    fn public_location(
        &self,
        _object: &school_file_platform::storage_provider::StoredObject,
    ) -> Result<Url, school_file_platform::storage_provider::StorageError> {
        Url::parse("https://public.example.test/file")
            .map_err(|_| school_file_platform::storage_provider::StorageError::OperationFailed)
    }
}
...
#[async_trait]
impl school_file_platform::storage_provider::StorageProvider for DeleteFailStorage {
    async fn check_readiness(
        &self,
    ) -> Result<(), school_file_platform::storage_provider::StorageError> {
        Ok(())
    ...
    async fn delete(
        ...
        Err(school_file_platform::storage_provider::StorageError::OperationFailed)
    }
...
#[async_trait]
impl school_file_platform::storage_provider::StorageProvider for BlockingGrantStorage {
    async fn check_readiness(
        &self,
    ) -> Result<(), school_file_platform::storage_provider::StorageError> {
        Ok(())
...
#[async_trait]
impl school_file_platform::malware_scanner::MalwareScanner for PreviewScanner {
    async fn scan(&self, _content: &[u8]) -> school_file_platform::malware_scanner::ScanOutcome {
        school_file_platform::malware_scanner::ScanOutcome::Clean
    }
}
...
) -> crate::models::CertificateTemplateDetail {
...
async fn attach_school_font_fixture(
    ...
    weight: u16,
    style: &str,
) -> school_fonts::models::SchoolFontSummary {
    ...
    template_service::attach_font_batch(
        ...
        actor,
        template_id,
        school_fonts::models::AttachSchoolFontBatchRequest {
...
async fn preview_manifest_preserves_exact_shared_font_style_and_omits_unused_fonts() {
    ...
    let platform = school_file_platform::platform_service::FilePlatform::new(
...
async fn background_rejects_wrong_relation_unready_pages_and_unsafe_geometry() {
    ...
    assert!(matches!(
        ...
        .await,
        Err(CertificateServiceError::Application(AppError::Forbidden(_)))
    ));
    ...
    assert!(matches!(
        ...
        .await,
        Err(CertificateServiceError::Application(AppError::Conflict(_)))
    ));
    ...
    assert!(matches!(
        ...
        .await,
        Err(CertificateServiceError::Application(
...
async fn active_request_locks_referenced_template_mutations() {
    ...
    assert!(matches!(
        locked,
        Err(CertificateServiceError::ResourceLocked {
...
async fn layout_save_synchronizes_shared_font_references_and_blocks_central_delete() {
    ...
    assert_eq!(
        school_fonts::services::delete(&pool, &manager, font.id)
            .await
            .unwrap(),
        school_fonts::services::SchoolFontDeleteOutcome::Conflict(
            school_fonts::models::SchoolFontDeleteConflict { reference_count: 1 }
        )
    );
    ...
    assert!(matches!(
        school_fonts::services::delete(&pool, &manager, font.id)
            .await
            .unwrap(),
        school_fonts::services::SchoolFontDeleteOutcome::Deleted { .. }
    ));
}
...
async fn concurrent_layout_save_and_shared_font_delete_leave_one_consistent_winner() {
    ...
    let delete = school_fonts::services::delete(&pool, &manager, font.id);
    let (save_result, delete_result) = tokio::join!(save, delete);

    match (save_result, delete_result) {
        (Ok(_), Ok(school_fonts::services::SchoolFontDeleteOutcome::Conflict(conflict))) => {
            assert_eq!(conflict.reference_count, 1)
        }
        (
            Err(CertificateServiceError::Application(
                AppError::Conflict(_) | AppError::NotFound(_),
            )),
            Ok(school_fonts::services::SchoolFontDeleteOutcome::Deleted { .. }),
        ) => {}
...
async fn concurrent_campaign_owner_transfer_blocks_stale_template_mutation() {
    ...
    assert!(
        matches!(
            mutation_result,
            Err(CertificateServiceError::Application(
                AppError::Conflict(_) | AppError::Forbidden(_),
            ))
        ),
...
async fn concurrent_campaign_purge_and_template_update_do_not_deadlock() {
    ...
    assert!(
        !matches!(
            update_result,
            Err(CertificateServiceError::Application(AppError::DbError(_)))
        ),
...
async fn purge_zero_file_campaign_requires_exact_snapshot_and_completes_atomically() {
    ...
    assert!(matches!(
        ...
        Err(AppError::ValidationError(message))
            if message == "certificate_purge_confirmation_mismatch"
    ));
...
async fn purge_counts_open_and_soft_deleted_rows_then_removes_file_metadata() {
    ...
    sqlx::query(
        "UPDATE certificates
         SET status = 'revoked', revoked_by = $2, revoked_at = NOW(),
             revocation_reason = 'Purge count fixture'
         WHERE campaign_id = $1",
    )
    .bind(campaign.id)
    .bind(actor.user_id)
    ...
    assert_eq!(impact.counts.candidate_count, 3);
    assert_eq!(impact.counts.request_count, 2);
    assert_eq!(impact.counts.open_request_count, 1);
    assert_eq!(impact.counts.issued_certificate_count, 0);
    assert_eq!(impact.counts.revoked_certificate_count, 1);
...
async fn purge_permission_is_school_or_exact_owner_unit_only() {
    ...
    let campaign_b_impact = purge_service::impact(&fixture.pool, &school_actor, campaign_b_id)
        .await
        .expect("school delete grant should inspect every owner scope");
    let platform = school_file_platform::platform_service::FilePlatform::new(
        Arc::new(PreviewStorage),
        Arc::new(PreviewScanner),
    );
    let unauthorized_start = purge_service::start(
        &fixture.pool,
        &actor,
        &platform,
        campaign_b_id,
        StartCertificateCampaignPurgeRequest {
            confirmation_name: campaign_b_impact.campaign_name,
            expected_updated_at: campaign_b_impact.updated_at,
            expected_impact: campaign_b_impact.counts,
        },
    )
    .await;
    for result in [
        purge_service::impact(&fixture.pool, &actor, campaign_b_id)
            .await
            .map(|_| ()),
        unauthorized_start.map(|_| ()),
        purge_service::status(&fixture.pool, &actor, campaign_b_id)
            .await
            .map(|_| ()),
        purge_service::retry(&fixture.pool, &actor, &platform, campaign_b_id)
            .await
            .map(|_| ()),
    ] {
        assert!(
            matches!(&result, Err(AppError::NotFound(_))),
            "out-of-scope purge endpoint must mask existence: {result:?}"
        );
    }
}
...
async fn purge_rejects_stale_impact_legal_hold_and_shared_files_without_partial_state() {
    ...
    assert!(matches!(
        ...
        Err(AppError::Conflict(message))
            if message == "certificate_purge_impact_changed"
    ));
    ...
    assert!(matches!(
        ...
        .await,
        Err(AppError::Conflict(message))
            if message == "certificate_purge_file_held"
    ));
    ...
    assert!(matches!(
        ...
        .await,
        Err(AppError::Conflict(message))
            if message == "certificate_purge_file_shared"
    ));
...
#[tokio::test]
async fn purge_rejects_files_referenced_by_profile_or_staff_achievement() {
    let (pool, actor, academic_year_id) =
        school_campaign_fixture("certificate_campaign_purge_external_file_refs", 3153).await;
    let campaign = campaign_service::create_campaign(
        &pool,
        &actor,
        campaign_create_payload(academic_year_id, None, "กิจกรรมไฟล์อ้างจากโดเมนอื่น"),
    )
    .await
    .unwrap();
    let template = template_service::create_template(
        ...
        campaign.id,
        CreateCertificateTemplateRequest {
            name: "แบบไฟล์อ้างจากโดเมนอื่น".to_string(),
            allowed_recipient_types: vec![RecipientType::External],
        },
    )
    .await
    ...
    let achievement_id: Uuid = sqlx::query_scalar(
        "INSERT INTO staff_achievements (
         ...
         ) VALUES ($1, 'ผลงานที่อ้างไฟล์ร่วม', CURRENT_DATE, $1, $2)
         RETURNING id",
    )
    .bind(actor.user_id)
    .bind(file_id)
    ...
    let state: (String, Option<Uuid>, Option<Uuid>, i64, i64) = sqlx::query_as(
        "SELECT campaign.status, users.profile_image_file_id,
                achievement.image_file_id,
                (SELECT COUNT(*) FROM certificate_campaign_purge_jobs
                 WHERE campaign_id = campaign.id),
                (SELECT COUNT(*) FROM files WHERE id = $3)
         FROM certificate_campaigns AS campaign
         JOIN users ON users.id = $1
         JOIN staff_achievements AS achievement ON achievement.id = $2
         WHERE campaign.id = $4",
    )
    .bind(actor.user_id)
    .bind(achievement_id)
    ...
    assert_eq!(
        state,
        ("draft".to_string(), Some(file_id), Some(file_id), 0, 1)
    );
}

#[tokio::test]
async fn purge_rejects_admission_logo_and_question_bank_file_consumers() {
    fn assert_file_shared(result: Result<CertificateCampaignPurgeStatus, AppError>) {
        assert!(matches!(
            result,
            Err(AppError::Conflict(message))
                if message == "certificate_purge_file_shared"
    ...
    let (pool, actor, academic_year_id) =
        school_campaign_fixture("certificate_campaign_purge_all_file_consumers", 3154).await;
    let campaign = campaign_service::create_campaign(
        &pool,
        &actor,
        campaign_create_payload(academic_year_id, None, "กิจกรรมไฟล์อ้างจากระบบอื่น"),
    )
    .await
    .unwrap();
    let template = template_service::create_template(
        ...
        campaign.id,
        CreateCertificateTemplateRequest {
            name: "แบบไฟล์อ้างจากระบบอื่น".to_string(),
            allowed_recipient_types: vec![RecipientType::External],
        },
    )
    .await
    ...
    let request = || StartCertificateCampaignPurgeRequest {
        confirmation_name: impact.campaign_name.clone(),
        expected_updated_at: impact.updated_at,
        expected_impact: impact.counts.clone(),
    };
    let platform = school_file_platform::platform_service::FilePlatform::new(
    ...
    let admission_round_id: Uuid = sqlx::query_scalar(
        "INSERT INTO admission_rounds (
         ...
         ) VALUES ($1, $2, 'รอบทดสอบผู้ใช้ไฟล์', CURRENT_DATE, CURRENT_DATE)
         RETURNING id",
    )
    .bind(academic_year_id)
    .bind(grade_level_id)
    ...
    let application_id: Uuid = sqlx::query_scalar(
        "INSERT INTO admission_applications (
         ...
         ) VALUES ($1, $2, 'encrypted-test-value', repeat('d', 64),
                   'ผู้สมัคร', 'ทดสอบ')
         RETURNING id",
    )
    .bind(admission_round_id)
    ...
    let document_id: Uuid = sqlx::query_scalar(
        "INSERT INTO admission_application_documents (application_id, file_id, doc_type)
         VALUES ($1, $2, 'purge_consumer_fixture')
         RETURNING id",
    )
    .bind(application_id)
    .bind(file_id)
    ...
    .unwrap();
    assert_file_shared(
        purge_service::start(&pool, &actor, &platform, campaign.id, request()).await,
    );
    ...
    let (subject_owner_id, subject_group_id): (Uuid, Uuid) = sqlx::query_as(
    ...
    let subject_id: Uuid = sqlx::query_scalar(
        "INSERT INTO subjects (
         ...
         VALUES (gen_random_uuid(), 'PURGE-FIXTURE', 'purge-fixture', $1, $2)
         RETURNING id",
    )
    ...
    .await
    .unwrap();
    assert_eq!(state, ("draft".to_string(), 1, 1));
}
...
async fn purge_provider_failure_is_durable_idempotent_and_retryable() {
    ...
    let failing_platform = school_file_platform::platform_service::FilePlatform::new(
    ...
    let premature_retry = purge_service::retry(&pool, &actor, &failing_platform, campaign.id).await;
    assert!(matches!(
        premature_retry,
        Err(AppError::Conflict(message))
            if message == "certificate_purge_retry_not_failed"
    ));
    sqlx::query(
    ...
    let healthy_platform = school_file_platform::platform_service::FilePlatform::new(
        Arc::new(PreviewStorage),
        Arc::new(PreviewScanner),
    );
...
async fn purging_campaign_is_visible_only_as_delete_progress_and_hides_normal_resources() {
    ...
    assert!(crate::access_policy::require_template_action(
        &pool,
        &actor,
        template.id,
    ...
    .await
    .unwrap();
    assert!(crate::file_relationships::record_template_upload(
        &pool,
        late_file_id,
        template.id,
...
async fn purging_campaign_hides_issued_surfaces_and_blocks_issue_and_revoke() {
    let _crypto_guard = school_crypto::test_env_lock();
    env::set_var(
        "ENCRYPTION_KEY",
    ...
    let public_render_error = render_service::public_manifest_rate_limited(
        ...
        &crate::verification_limiter::CertificateVerificationLimiter::new(),
        &public_receipt,
    )
    .await
    .unwrap_err();
...
async fn create_ready_external_request_candidate(
    ...
    crate::models::CertificateTemplateDetail,
    crate::models::CertificateCandidateDetail,
) {
    let template = create_ready_candidate_template(
        pool,
        actor,
...
fn update_external_candidate_payload(
    candidate: &crate::models::CertificateCandidateDetail,
    first_name: &str,
) -> UpdateCertificateCandidateRequest {
    UpdateCertificateCandidateRequest {
...
async fn active_request_locks_only_selected_candidates_and_referenced_templates() {
    ...
    assert!(matches!(
        ...
        .await,
        Err(CertificateServiceError::ResourceLocked { request_id: None })
    ));
...
async fn issue_public_verification_fixture(
    test_name: &str,
    year: i32,
) -> (PgPool, crate::models::IssuedCertificateSummary, Uuid) {
...
async fn public_verification_rate_limits_after_six_failed_attempts_for_one_target() {
    ...
    let limiter = crate::verification_limiter::CertificateVerificationLimiter::new();
...
async fn public_verification_success_is_allowlisted_and_issues_a_short_lived_receipt() {
    ...
    let encrypted_proof: String =
        sqlx::query_scalar("SELECT qr_proof_encrypted FROM certificates WHERE id = $1")
            ...
            .await
            .unwrap();
    let proof = school_crypto::decrypt(&encrypted_proof).unwrap();
...
async fn issued_lists_and_details_enforce_exact_read_and_separate_download_scope() {
    ...
    let list = issuance_service::list_campaign_certificates(
        ...
        &reader,
        campaign.id,
        crate::models::IssuedCertificateListQuery {
...
async fn issued_manifest_uses_canonical_proof_url_and_refuses_revoked_certificates() {
    ...
    let blocking_platform = Arc::new(school_file_platform::platform_service::FilePlatform::new(
