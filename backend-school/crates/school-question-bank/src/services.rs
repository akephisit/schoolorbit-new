...
use crate::access_policy::{self as question_bank_access_policy, QuestionBankAccess};
use crate::models::{
...
/// Loads the minimal ownership scope required by application-level file authorization.
pub async fn fetch_question_scope(
    pool: &PgPool,
    question_id: Uuid,
...
/// Loads the file references used by the application-level File Platform adapter.
pub async fn fetch_question_file_ids(
    pool: &PgPool,
    question_id: Uuid,
) -> Result<HashSet<Uuid>, AppError> {
