...
pub async fn summary_for_actor(
    ...
    crate::policy::learner_evaluation::require_student_summary_access(
        pool,
        actor,
...
pub async fn summarize_student_term_in_transaction(
...
pub async fn summarize_student_terms_in_transaction(
    tx: &mut Transaction<'_, Postgres>,
    ctx: &EvaluationContext,
    students: &[Uuid],
) -> Result<BTreeMap<Uuid, StudentEvaluationSummary>, AppError> {
    validate_context(tx, ctx).await?;
    school_academic_core::services::student_year_batch::validate_student_year_batch(
        tx,
        ctx.academic_year_id,
        students,
    )
