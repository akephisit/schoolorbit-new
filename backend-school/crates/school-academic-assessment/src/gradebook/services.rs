use super::models::*;
use crate::policy::gradebook as policy;
use crate::ports::ResultLockPort;
use bigdecimal::BigDecimal;
use school_academic_core::services::lifecycle_guard::{self, AcademicWriteState};
use school_authorization::{AcademicResourceListFilter, ActorContext};
...
pub async fn resolve_phase_id(
    ...
    let phase_code = crate::assessment::models::AssessmentPhaseCode::try_from(phase_code)
...
pub(super) async fn begin_scope<'a>(
    result_locks: Option<&(dyn ResultLockPort + Send + Sync)>,
    pool: &'a PgPool,
    actor: &ActorContext,
    group: Uuid,
    phase: Uuid,
    ...
    if write {
        ...
        result_locks
            .expect("Gradebook writes require the Results lock adapter")
