...
"#,
    ...
    async fn update_profile_preserves_replaces_and_clears_profile_image() {
        ...
        let mut transaction = pool.begin().await.expect("profile update should begin");
        let preserved = update_profile(&mut transaction, user_id, profile_update(None))
            .await
        ...
        let preserved_user = find_user_by_id(&pool, user_id)
            .await
            .expect("preserved user should load");
        assert_eq!(preserved_user.profile_image_file_id, Some(old_file_id));
        assert_eq!(preserved.replaced_file_id, None);

        let mut transaction = pool.begin().await.expect("profile update should begin");
        let replaced = update_profile(
            &mut transaction,
            user_id,
            profile_update(Some(Some(new_file_id))),
        )
        .await
        ...
        transaction
            ...
            .await
            .expect("profile update should commit");
        let replaced_user = find_user_by_id(&pool, user_id)
            .await
            .expect("replaced user should load");
        assert_eq!(replaced_user.profile_image_file_id, Some(new_file_id));
        assert_eq!(replaced.replaced_file_id, Some(old_file_id));

        let mut transaction = pool.begin().await.expect("profile update should begin");
        let cleared = update_profile(&mut transaction, user_id, profile_update(Some(None)))
            .await
        ...
        transaction
            ...
            .await
            .expect("profile update should commit");
        let cleared_user = find_user_by_id(&pool, user_id)
            .await
            .expect("cleared user should load");
        assert_eq!(cleared_user.profile_image_file_id, None);
