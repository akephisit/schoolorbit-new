...
pub struct DerivedTermIdentity {
    pub code: String,
    pub name: String,
}

pub fn derive_academic_year_name(year: i32, custom_name: Option<&str>) -> Result<String, AppError> {
    if year <= 0 {
...
pub async fn append_audit<T: serde::Serialize>(
    transaction: &mut Transaction<'_, Postgres>,
    event_code: &str,
    entity_type: &str,
    entity_id: Uuid,
