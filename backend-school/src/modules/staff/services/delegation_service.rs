...
pub async fn list_delegations(pool: &PgPool, department_id: Uuid) -> Result<Vec<DelegationItem>, AppError> {
    ...
    Ok(rows.into_iter().map(|row| DelegationItem {
        id: row.get("id"),
...
pub async fn is_department_head(pool: &PgPool, user_id: Uuid, department_id: Uuid) -> Result<bool, AppError> {
...
pub async fn is_department_member(pool: &PgPool, user_id: Uuid, department_id: Uuid) -> Result<bool, AppError> {
    let r: bool = sqlx::query_scalar(
        "SELECT EXISTS(
             SELECT 1 FROM department_members
...
pub async fn create_delegation(
    ...
    .bind(from_user_id).bind(to_user_id).bind(permission_id)
    .bind(department_id).bind(reason).bind(expires_at)
    .fetch_one(pool).await?;
    Ok(id)
}

pub async fn get_delegation_users(pool: &PgPool, id: Uuid) -> Result<Option<(Uuid, Uuid)>, AppError> {
    let row = sqlx::query(
        "SELECT from_user_id, to_user_id FROM permission_delegations WHERE id = $1 AND revoked_at IS NULL"
    )
    .bind(id).fetch_optional(pool).await?;

    Ok(row.map(|r| (r.get::<Uuid, _>("from_user_id"), r.get::<Uuid, _>("to_user_id"))))
}
