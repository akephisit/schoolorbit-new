...
pub async fn list_members(pool: &PgPool, department_id: Uuid, include_children: bool) -> Result<Vec<DeptMemberItem>, AppError> {
    ...
    Ok(rows.into_iter().map(|r| DeptMemberItem {
...
pub async fn already_member(pool: &PgPool, user_id: Uuid, department_id: Uuid) -> Result<bool, AppError> {
    ...
    .bind(user_id).bind(department_id).fetch_one(pool).await?;
    Ok(r)
}

pub async fn add_member(
    pool: &PgPool, user_id: Uuid, department_id: Uuid,
    position: &str, is_primary: bool, responsibilities: Option<String>,
) -> Result<(), AppError> {
    sqlx::query(
    ...
    .bind(user_id).bind(department_id).bind(position)
    .bind(is_primary).bind(responsibilities)
    .execute(pool).await?;
    Ok(())
}

pub async fn update_member(
    pool: &PgPool, department_id: Uuid, user_id: Uuid,
    position: &str, is_primary: bool, responsibilities: Option<String>,
    ...
    .bind(position).bind(is_primary).bind(responsibilities)
    .bind(new_department_id).bind(user_id).bind(department_id)
    .execute(pool).await?;
    Ok(result.rows_affected())
}

pub async fn remove_member(pool: &PgPool, department_id: Uuid, user_id: Uuid) -> Result<(), AppError> {
    sqlx::query(
        "UPDATE department_members SET ended_at = CURRENT_DATE
         WHERE user_id = $1 AND department_id = $2
           AND (ended_at IS NULL OR ended_at > CURRENT_DATE)"
    )
    .bind(user_id).bind(department_id).execute(pool).await?;
    Ok(())
}
