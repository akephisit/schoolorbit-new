...
pub async fn list_subjects(pool: &PgPool, filter: SubjectFilter, dept_group_id: Option<Uuid>) -> Result<Vec<Subject>, AppError> {
    ...
    if let Some(active) = filter.active_only {
        if active { query.push_str(" AND s.is_active = true"); }
    }

    let effective_group_id: Option<Uuid> = if dept_group_id.is_some() { dept_group_id } else { filter.group_id };
    if effective_group_id.is_some() { idx += 1; query.push_str(&format!(" AND s.group_id = ${idx}")); }
    if filter.subject_type.is_some() { idx += 1; query.push_str(&format!(" AND s.type = ${idx}")); }
    ...
    if search_pattern.is_some() {
        idx += 1;
        query.push_str(&format!(" AND (s.code ILIKE ${idx} OR s.name_th ILIKE ${idx} OR s.name_en ILIKE ${idx})"));
    }
...
pub async fn create_subject(pool: &PgPool, payload: CreateSubjectRequest) -> Result<Subject, AppError> {
    let exists: Option<bool> = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM subjects WHERE code = $1 AND start_academic_year_id = $2)"
    )
    .bind(&payload.code).bind(payload.start_academic_year_id)
    .fetch_one(pool).await.unwrap_or(Some(false));

    if exists.unwrap_or(false) {
        let year_name: Option<String> = sqlx::query_scalar("SELECT name FROM academic_years WHERE id = $1")
            .bind(payload.start_academic_year_id).fetch_optional(pool).await.unwrap_or(None);
        return Err(AppError::BadRequest(format!(
            "รหัสวิชา {} {} มีอยู่ในระบบแล้ว",
            payload.code, year_name.unwrap_or_else(|| "ในปีการศึกษานี้".to_string())
        )));
    }
    ...
    if let Some(level_ids) = &payload.grade_level_ids {
        for lid in level_ids {
            sqlx::query("INSERT INTO subject_grade_levels (subject_id, grade_level_id) VALUES ($1, $2)")
                .bind(subject.id).bind(lid).execute(&mut *tx).await
                .map_err(|e| {
    ...
    if let Some(team) = &payload.default_instructors {
        sqlx::query("DELETE FROM subject_default_instructors WHERE subject_id = $1")
            .bind(subject.id).execute(&mut *tx).await
        ...
        for t in team {
            ...
            .bind(subject.id).bind(t.instructor_id).bind(&t.role).execute(&mut *tx).await
...
pub async fn update_subject(pool: &PgPool, id: Uuid, payload: UpdateSubjectRequest) -> Result<Subject, AppError> {
    let mut tx = pool.begin().await.map_err(|_| AppError::InternalServerError("Transaction failed".to_string()))?;
    ...
    if let Some(level_ids) = &payload.grade_level_ids {
        ...
        for lid in level_ids {
            sqlx::query("INSERT INTO subject_grade_levels (subject_id, grade_level_id) VALUES ($1, $2)")
                .bind(id).bind(lid).execute(&mut *tx).await
    ...
    if let Some(team) = &payload.default_instructors {
        ...
        for t in team {
            ...
            .bind(id).bind(t.instructor_id).bind(&t.role).execute(&mut *tx).await
            .map_err(|e| AppError::InternalServerError(format!("Failed to save team: {}", e)))?;
        }
    }
...
pub async fn list_subject_default_instructors(pool: &PgPool, subject_id: Uuid) -> Result<Vec<SubjectDefaultInstructor>, AppError> {
...
pub async fn add_subject_default_instructor(pool: &PgPool, subject_id: Uuid, body: AddSubjectDefaultInstructorRequest) -> Result<(), AppError> {
    ...
    if role == "primary" {
        ...
        ).bind(subject_id).bind(body.instructor_id).execute(&mut *tx).await
        .map_err(|e| AppError::InternalServerError(e.to_string()))?;
    }
    ...
    sqlx::query(
        "INSERT INTO subject_default_instructors (subject_id, instructor_id, role)
         VALUES ($1, $2, $3)
         ON CONFLICT (subject_id, instructor_id) DO UPDATE SET role = EXCLUDED.role"
    ).bind(subject_id).bind(body.instructor_id).bind(&role).execute(&mut *tx).await
    .map_err(|e| AppError::InternalServerError(e.to_string()))?;
...
pub async fn remove_subject_default_instructor(pool: &PgPool, subject_id: Uuid, instructor_id: Uuid) -> Result<(), AppError> {
    sqlx::query(
        "DELETE FROM subject_default_instructors WHERE subject_id = $1 AND instructor_id = $2"
    ).bind(subject_id).bind(instructor_id).execute(pool).await
    .map_err(|e| AppError::InternalServerError(e.to_string()))?;
    Ok(())
}

pub async fn update_subject_default_instructor_role(pool: &PgPool, subject_id: Uuid, instructor_id: Uuid, role: &str) -> Result<(), AppError> {
    if role != "primary" && role != "secondary" {
        return Err(AppError::BadRequest("role must be 'primary' or 'secondary'".to_string()));
    }
    ...
    if role == "primary" {
        sqlx::query(
            "UPDATE subject_default_instructors SET role = 'secondary'
             WHERE subject_id = $1 AND instructor_id <> $2 AND role = 'primary'"
        ).bind(subject_id).bind(instructor_id).execute(&mut *tx).await
        .map_err(|e| AppError::InternalServerError(e.to_string()))?;
    }
    ...
    ).bind(subject_id).bind(instructor_id).bind(role).execute(&mut *tx).await
    .map_err(|e| AppError::InternalServerError(e.to_string()))?;
...
pub async fn batch_list_subject_default_instructors(
    ...
    .bind(&subject_ids).fetch_all(pool).await
    .map_err(|e| AppError::InternalServerError(e.to_string()))?;
