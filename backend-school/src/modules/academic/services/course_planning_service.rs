...
pub async fn list_classroom_courses(pool: &PgPool, query: &PlanQuery) -> Result<Vec<ClassroomCourse>, AppError> {
    ...
    if query.classroom_id.is_some() { idx += 1; sql.push_str(&format!(" AND cc.classroom_id = ${idx}")); has_filter = true; }
    ...
    if query.academic_semester_id.is_some() { idx += 1; sql.push_str(&format!(" AND cc.academic_semester_id = ${idx}")); has_filter = true; }
    if query.subject_id.is_some() { idx += 1; sql.push_str(&format!(" AND cc.subject_id = ${idx}")); has_filter = true; }
    ...
    if let Some(id) = query.classroom_id { q = q.bind(id); }
    if let Some(id) = query.instructor_id { q = q.bind(id); }
    if let Some(id) = query.academic_semester_id { q = q.bind(id); }
    if let Some(id) = query.subject_id { q = q.bind(id); }

    q.fetch_all(pool).await
        .map_err(|e| { eprintln!("Failed to fetch courses: {}", e); AppError::InternalServerError("Failed to fetch courses".to_string()) })
}
...
pub async fn assign_courses(pool: &PgPool, payload: AssignCoursesRequest) -> Result<i64, AppError> {
    let exists: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM class_rooms WHERE id = $1)")
        .bind(payload.classroom_id).fetch_one(pool).await.unwrap_or(false);
...
pub async fn update_course(pool: &PgPool, id: Uuid, payload: UpdateCourseRequest) -> Result<(), AppError> {
    sqlx::query(
    ...
    .bind(payload.primary_instructor_id).bind(payload.settings).bind(id)
    .execute(pool).await
    .map_err(|e| { eprintln!("Update error: {}", e); AppError::InternalServerError("Failed to update course".to_string()) })?;
    Ok(())
}

pub async fn batch_list_course_instructors(
    pool: &PgPool, ids: &[Uuid],
    ...
    for row in rows { grouped.entry(row.classroom_course_id).or_default().push(row); }
    Ok(grouped)
}

pub async fn list_course_instructors(pool: &PgPool, course_id: Uuid) -> Result<Vec<CourseInstructor>, AppError> {
    sqlx::query_as(
        r#"SELECT cci.*, concat(u.first_name, ' ', u.last_name) AS instructor_name
           FROM classroom_course_instructors cci
           JOIN users u ON u.id = cci.instructor_id
...
pub async fn add_course_instructor(pool: &PgPool, course_id: Uuid, instructor_id: Uuid, role: &str) -> Result<(), AppError> {
    ...
    .bind(course_id).bind(instructor_id).bind(role)
...
pub async fn remove_course_instructor(pool: &PgPool, course_id: Uuid, instructor_id: Uuid) -> Result<(), AppError> {
    let mut tx = pool.begin().await.map_err(|e| AppError::InternalServerError(e.to_string()))?;
...
pub async fn update_course_instructor_role(pool: &PgPool, course_id: Uuid, instructor_id: Uuid, role: &str) -> Result<(), AppError> {
    let mut tx = pool.begin().await.map_err(|e| AppError::InternalServerError(e.to_string()))?;
...
pub async fn list_classroom_activities(pool: &PgPool, classroom_id: Uuid, semester_id: Uuid) -> Result<Vec<ClassroomActivity>, AppError> {
    sqlx::query_as(
    ...
    .bind(classroom_id).bind(semester_id).fetch_all(pool).await
    .map_err(|e| { eprintln!("list_classroom_activities error: {e}"); AppError::InternalServerError("เกิดข้อผิดพลาด".to_string()) })
}

pub async fn remove_classroom_from_slot(pool: &PgPool, classroom_id: Uuid, slot_id: Uuid) -> Result<(), AppError> {
    sqlx::query("DELETE FROM activity_slot_classrooms WHERE classroom_id = $1 AND slot_id = $2")
        .bind(classroom_id).bind(slot_id).execute(pool).await
        .map_err(|e| AppError::InternalServerError(e.to_string()))?;
    Ok(())
}
