...
pub fn academic_routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/levels/{id}",
            axum::routing::delete(handlers::delete_grade_level),
        )
        // Academic Years
        ...
        .route(
            "/years/{id}/levels",
            get(handlers::get_year_levels).put(handlers::update_year_levels),
        )
        // Semesters
        .route("/semesters", post(handlers::create_semester))
        .route(
            "/semesters/{id}",
            put(handlers::update_semester).delete(handlers::delete_semester),
        )
        // Classrooms
        .route(
            "/classrooms",
        ...
        .route(
            ...
            get(handlers::get_class_enrollments),
        )
        .route(
            "/enrollments/{id}",
            axum::routing::delete(handlers::remove_enrollment),
        )
        .route(
            "/enrollments/{id}/number",
            put(handlers::update_enrollment_number),
        )
        .route(
            "/enrollments/class/{id}/auto-number",
            post(handlers::auto_assign_class_numbers),
        )
        // Curriculum: Subjects
        .route(
            "/subjects/groups",
            get(handlers::subjects::list_subject_groups),
        )
        // Batch endpoint — MUST be registered BEFORE `/subjects/{id}/...`
        // so Axum doesn't match `default-instructors` as `id`.
        .route(
            "/subjects/default-instructors",
            get(handlers::subjects::batch_list_subject_default_instructors),
        )
        .route(
            "/subjects",
        ...
        .route(
            ...
            get(handlers::course_planning::batch_list_course_instructors),
        )
        .route(
        ...
        .route(
            "/periods",
            get(handlers::timetable::list_periods).post(handlers::timetable::create_period),
        )
        .route(
            "/periods/reorder",
            post(handlers::timetable::reorder_periods),
        )
        .route(
            "/periods/{id}",
            put(handlers::timetable::update_period).delete(handlers::timetable::delete_period),
        )
        // Timetable: Entries
        .route(
            "/timetable",
        ...
        .route(
            "/study-plans",
            get(handlers::study_plans::list_study_plans)
                .post(handlers::study_plans::create_study_plan),
        )
        .route(
            "/study-plans/{id}",
            get(handlers::study_plans::get_study_plan)
                .put(handlers::study_plans::update_study_plan)
                .delete(handlers::study_plans::delete_study_plan),
        )
        // Study Plan Versions
        .route(
            "/study-plan-versions",
            get(handlers::study_plans::list_study_plan_versions)
                .post(handlers::study_plans::create_study_plan_version),
        )
        .route(
            "/study-plan-versions/{id}",
            get(handlers::study_plans::get_study_plan_version)
                .put(handlers::study_plans::update_study_plan_version)
                .delete(handlers::study_plans::delete_study_plan_version),
        )
        // Study Plan Subjects
        .route(
            "/study-plan-versions/{id}/subjects",
            get(handlers::study_plans::list_study_plan_subjects)
                .post(handlers::study_plans::add_subjects_to_version),
        )
        .route(
            "/study-plan-subjects/{id}",
            axum::routing::delete(handlers::study_plans::delete_study_plan_subject),
        )
        // Bulk: Generate Courses from Study Plan
        .route(
            "/planning/generate-from-plan",
            post(handlers::study_plans::generate_courses_from_plan),
        )
        // Study Plan Activities (template)
        .route(
            "/study-plan-versions/{id}/activities",
            get(handlers::study_plans::list_plan_activities)
                .post(handlers::study_plans::add_plan_activity),
        )
        .route(
            "/study-plan-activities/{id}",
        ...
        .route(
            ...
            post(handlers::scheduling::auto_schedule_timetable),
        )
        .route(
            "/scheduling/jobs",
            get(handlers::scheduling::list_scheduling_jobs),
        )
        .route(
            "/scheduling/jobs/{id}",
            get(handlers::scheduling::get_scheduling_job),
        )
        .route(
            "/scheduling/jobs/{id}/undo",
            post(handlers::scheduling::undo_scheduling_job),
        )
        // Instructor Preferences
        .route(
            "/instructor-preferences",
            post(handlers::scheduling::create_instructor_preference),
        )
        // Instructor Room Assignments
        .route(
            "/instructor-rooms",
            post(handlers::scheduling::create_instructor_room_assignment),
        )
        // Locked Slots
        .route(
            "/timetable/locked-slots",
            post(handlers::scheduling::create_locked_slot)
                .get(handlers::scheduling::list_locked_slots),
        )
        .route(
            "/timetable/locked-slots/{id}",
            axum::routing::delete(handlers::scheduling::delete_locked_slot),
        )
