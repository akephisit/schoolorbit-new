...
pub async fn complete_enrollment(
    ...
    let (before_status, before_round, before_year): (String, Uuid, Uuid) = sqlx::query_as(
        "SELECT application.status,application.admission_round_id,round.academic_year_id
    ...
    .await?
    .ok_or_else(|| AppError::NotFound("ไม่พบใบสมัคร".into()))?;
    let receipt_only = before_status == "enrolled";
    if !receipt_only {
        lifecycle_guard::require_year_write_exclusive(&mut tx, before_year).await?;
    }
    ...
    if receipt_only || application.admission_round_id != before_round {
        return Err(AppError::Conflict(
            "สถานะหรือรอบรับสมัครเปลี่ยนแล้ว กรุณาโหลดข้อมูลใหม่".into(),
        ));
    }
    ...
    if placement_context.academic_year_id != before_year {
        return Err(AppError::Conflict(
            "ปีการศึกษาของรอบรับสมัครเปลี่ยนแล้ว กรุณาโหลดข้อมูลใหม่".into(),
        ));
    }
...
mod tests {
    ...
    use crate::{
        ...
        test_helpers::{create_named_test_pool_with_max_connections, create_test_user},
    };
    use chrono::TimeZone;
