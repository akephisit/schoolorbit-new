...
pub async fn get_public_round_info(pool: &PgPool, id: Uuid) -> Result<(AdmissionRound, Vec<AdmissionTrack>), AppError> {
...
pub async fn create_round(pool: &PgPool, payload: CreateAdmissionRoundRequest) -> Result<AdmissionRound, AppError> {
    ...
    .bind(payload.academic_year_id).bind(payload.grade_level_id)
...
pub async fn update_round(pool: &PgPool, id: Uuid, payload: UpdateAdmissionRoundRequest) -> Result<AdmissionRound, AppError> {
    sqlx::query_as::<_, AdmissionRound>(
...
pub async fn update_round_status(pool: &PgPool, id: Uuid, status: &str) -> Result<(), AppError> {
    let valid = ["draft", "open", "exam_announced", "announced", "enrolling", "closed"];
    if !valid.contains(&status) {
        return Err(AppError::BadRequest(format!("สถานะ '{}' ไม่ถูกต้อง", status)));
    }
    if status == "draft" {
        return Err(AppError::BadRequest("ไม่สามารถเปลี่ยนกลับไปสถานะ 'ร่าง' ได้".to_string()));
    }

    let exists: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM admission_rounds WHERE id = $1)")
        .bind(id).fetch_one(pool).await.unwrap_or(false);
    if !exists {
...
pub async fn delete_round(pool: &PgPool, id: Uuid) -> Result<(), AppError> {
    ...
    if !file_rows.is_empty() {
        ...
        sqlx::query("DELETE FROM files WHERE id = ANY($1)")
            .bind(&file_ids).execute(pool).await.ok();
    }
...
pub async fn toggle_round_visibility(pool: &PgPool, id: Uuid, is_visible: bool) -> Result<bool, AppError> {
...
pub async fn list_exam_subjects(pool: &PgPool, round_id: Uuid) -> Result<Vec<AdmissionExamSubject>, AppError> {
...
pub async fn create_exam_subject(pool: &PgPool, round_id: Uuid, payload: CreateExamSubjectRequest) -> Result<AdmissionExamSubject, AppError> {
    sqlx::query_as::<_, AdmissionExamSubject>(
...
pub async fn update_exam_subject(pool: &PgPool, id: Uuid, payload: UpdateExamSubjectRequest) -> Result<AdmissionExamSubject, AppError> {
    sqlx::query_as::<_, AdmissionExamSubject>(
...
pub async fn create_track(pool: &PgPool, round_id: Uuid, payload: CreateAdmissionTrackRequest) -> Result<AdmissionTrack, AppError> {
    ...
    .bind(round_id).bind(payload.study_plan_id).bind(&payload.name)
    .bind(payload.capacity_override).bind(scoring_ids)
...
pub async fn update_track(pool: &PgPool, id: Uuid, payload: UpdateAdmissionTrackRequest) -> Result<AdmissionTrack, AppError> {
...
pub async fn delete_track(pool: &PgPool, id: Uuid) -> Result<(), AppError> {
    ...
    if count > 0 {
        return Err(AppError::BadRequest(format!("ไม่สามารถลบสายที่มีใบสมัครอยู่แล้ว ({} ใบ)", count)));
    }
