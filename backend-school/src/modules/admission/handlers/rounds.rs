...
use crate::AppState;
use school_admission::rounds::{self as round_service, *};
...
struct PublicRoundInfoData {
    round: AdmissionRound,
    tracks: Vec<AdmissionTrack>,
}

#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct RoundVisibilityData {
    is_visible: bool,
}

#[derive(Serialize)]
struct TrackCapacityData {
    rooms: Vec<round_service::RoomCapacityRow>,
    room_count: usize,
}
...
pub async fn get_public_round_info(
    ...
    Ok(Json(ApiResponse::ok(PublicRoundInfoData { round, tracks })))
}
...
pub async fn toggle_round_visibility(
    ...
    Ok(Json(ApiResponse::ok(RoundVisibilityData {
        is_visible: updated,
    }))
    .into_response())
}
...
pub async fn delete_subject(
    ...
    Ok(Json(ApiResponse::empty_with_message("ลบวิชาแล้ว")).into_response())
}
...
pub async fn list_tracks(
    ...
    Ok(Json(ApiResponse::ok(tracks)).into_response())
}
...
pub async fn create_track(
    ...
    Ok((StatusCode::CREATED, Json(ApiResponse::ok(track))).into_response())
}
...
pub async fn update_track(
    ...
    Ok(Json(ApiResponse::ok(track)).into_response())
}
...
pub async fn delete_track(
    ...
    Ok(Json(ApiResponse::empty_with_message("ลบสายการเรียนแล้ว")).into_response())
}
...
pub async fn get_track_capacity(
    ...
    Ok(Json(ApiResponse::ok(TrackCapacityData { rooms, room_count })).into_response())
}
