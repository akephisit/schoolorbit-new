...
/// Activity results have no school-wide entry-window override. School managers
/// may inspect every group, but an outcome is still entered by a teacher who is
/// currently assigned to that activity group.
pub fn can_enter_activity(actor: &ActorContext, assigned: bool) -> bool {
    assigned
        && (can_manage_school(actor)
            || actor.has_permission(codes::ACADEMIC_RESULT_MANAGE_ASSIGNED))
}

pub fn can_confirm_course(actor: &ActorContext, primary: bool) -> bool {
    primary && can_manage_group(actor, true)
}

pub fn can_confirm_activity(actor: &ActorContext, primary: bool) -> bool {
    primary && can_manage_group(actor, true)
}
...
pub async fn list_access(
    ...
    let filter = resource_access_policy::resolve_academic_resource_list_filter(
        ...
        AcademicResourcePermissions {
            assigned: &[
                codes::ACADEMIC_RESULT_READ_ASSIGNED,
                codes::ACADEMIC_RESULT_MANAGE_ASSIGNED,
            ],
            organization_unit: &[codes::ACADEMIC_RESULT_READ_ORGANIZATION_UNIT],
            organization_tree: &[],
            school: &[
                codes::ACADEMIC_RESULT_READ_SCHOOL,
                codes::ACADEMIC_RESULT_MANAGE_SCHOOL,
                codes::ACADEMIC_RESULT_LOCK_SCHOOL,
                codes::ACADEMIC_RESULT_CORRECT_SCHOOL,
            ],
        },
    )
    .await?;
    ...
    {
        return Err(AppError::Forbidden("Academic result access denied".into()));
    }
    Ok(filter)
}
...
mod tests {
    ...
    #[test]
    fn exact_unit_and_assigned_read_scopes_are_unioned() {
    ...
    #[test]
    fn school_management_does_not_replace_primary_role() {
        let actor = ActorContext {
            user_id: Uuid::new_v4(),
            permissions: vec![codes::ACADEMIC_RESULT_MANAGE_SCHOOL.into()],
        };
        assert!(can_manage_group(&actor, false));
        assert!(!can_confirm_course(&actor, false));
        assert!(!can_confirm_activity(&actor, false));
        assert!(!can_enter_activity(&actor, false));
        assert!(can_enter_activity(&actor, true));
        assert!(can_confirm_course(&actor, true));
    }
}
