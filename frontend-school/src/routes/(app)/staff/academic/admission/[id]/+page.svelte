<script lang="ts">
	...
	async function handleShowScoresToggle() {
		if (!canScoreAdmission) {
			toast.error('ไม่มีสิทธิ์ตั้งค่าการแสดงคะแนน');
			return;
		}
		if (!round) return;
	...
	async function confirmStatusChange() {
		if (!canManageAdmission) {
			toast.error('ไม่มีสิทธิ์เปลี่ยนสถานะรอบรับสมัคร');
			return;
		}
	...
	async function saveTrack() {
		if (!canManageAdmission) {
			toast.error('ไม่มีสิทธิ์จัดการสายการเรียน');
			return;
		}
		if (!id) return;
		savingTrack = true;
	...
	async function confirmDeleteTrack() {
		if (!canManageAdmission) {
			toast.error('ไม่มีสิทธิ์ลบสายการเรียน');
			return;
		}
	...
	async function saveSubject() {
		if (!canManageAdmission) {
			toast.error('ไม่มีสิทธิ์จัดการวิชาสอบ');
			return;
		}
		if (!id) return;
		savingSubject = true;
	...
	async function confirmDeleteSubject() {
		if (!canManageAdmission) {
			toast.error('ไม่มีสิทธิ์ลบวิชาสอบ');
			return;
		}
	...
	async function saveReportConfig() {
		if (!canManageAdmission) {
			toast.error('ไม่มีสิทธิ์ตั้งค่ารายงาน');
			return;
		}
		if (!id) return;
