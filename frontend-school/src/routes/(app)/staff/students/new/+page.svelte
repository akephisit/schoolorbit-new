<script lang="ts">
	...
	const canHandleStudentPii = $derived(
		$can.has(PERMISSIONS.STUDENT_CREATE_ALL) ||
			$can.hasAny(
	...
	function validateForm(): boolean {
		...
		if (canHandleStudentPii && formData.national_id && !/^\d{13}$/.test(formData.national_id)) {
		...
		formData.parents.forEach((parent, index) => {
			...
			if (canHandleStudentPii && parent.national_id && !/^\d{13}$/.test(parent.national_id)) {
	...
	async function handleSubmit() {
		...
		try {
			...
			const cleanedPayload = {
				...payload,
				national_id: canHandleStudentPii ? payload.national_id || undefined : undefined,
				...
				parents: formData.parent_enabled
					? formData.parents.map((p) => ({
							...
							national_id: canHandleStudentPii ? p.national_id || undefined : undefined
						}))
					: undefined
			};
				...
				<div class="space-y-4">
					<div>
						...
						<Input
							...
							placeholder="66001"
							class={errors.student_id
								? 'border-destructive font-mono text-lg'
								: 'font-mono text-lg'}
							disabled={loading}
							required
						/>
						...
						{#each formData.parents as parent, i (i)}
							<div class="p-4 border rounded-lg relative bg-muted/30">
								...
								<h3 class="text-sm font-medium mb-4 flex items-center gap-2">
									...
									ผู้ปกครองคนที่ {i + 1}
									{#if i === 0}<span class="text-xs text-muted-foreground">(ผู้ปกครองหลัก)</span
										>{/if}
								</h3>
