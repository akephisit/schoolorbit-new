<script lang="ts">
	...
	async function loadCalendar() {
		...
		try {
			...
			const nextEvents = await listChildCalendarEvents(studentId, {
	...
	async function loadHistory(requestedStudentId: string) {
		...
		try {
			...
			const nextEvents = await listChildCalendarEvents(requestedStudentId, {
				academicYearId: selectedYearId,
				academicTermId: selectedTermId || undefined,
				...calendarGridRange(selectedMonth)
			});
	...
	async function updateUrl(yearId: string, termId: string) {
		...
		await goto(resolve(`/parent/student/${studentId}/calendar?${query.toString()}`), {
			noScroll: true,
			keepFocus: true
		});
	}
...
>
	<div class="flex flex-wrap gap-3 rounded-xl border bg-card p-4">
		<div class="min-w-52 space-y-2">
			<Label for="parent-calendar-year">ปีการศึกษา</Label>
			<Select.Root
				type="single"
			...
			>
				<Select.Trigger id="parent-calendar-year" class="w-full">
		...
		<div class="min-w-52 space-y-2">
			<Label for="parent-calendar-term">ภาคเรียน</Label>
			<Select.Root
				type="single"
			...
			>
				<Select.Trigger id="parent-calendar-term" class="w-full">
