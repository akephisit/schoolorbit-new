<script lang="ts">
	...
	async function loadAvailableRoles() {
		try {
			...
			if (response.success && response.data) {
				availableRoles = response.data.filter((r) => r.is_active);
			}
		} catch (error) {
	...
	<Card>
		<CardHeader>
			<div class="flex items-center justify-between">
				...
				<Button on:click={() => (showAssignDialog = true)} size="sm" class="gap-2">
					<Plus class="h-4 w-4" />
					เพิ่มบทบาท
				</Button>
			</div>
		...
		<CardContent>
			...
			{:else if userRoles.length === 0}
				<div class="text-center py-8">
					...
					<Button
						on:click={() => (showAssignDialog = true)}
						variant="outline"
						size="sm"
						class="mt-4 gap-2"
					>
						...
						{#if role}
							<div class="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50">
								...
								<Button
									variant="ghost"
									size="sm"
									on:click={() => handleRemoveRole(role.id, role.name)}
...
<Dialog bind:open={showAssignDialog}>
	<DialogContent>
		...
		<DialogFooter>
			<Button variant="outline" on:click={() => (showAssignDialog = false)}>ยกเลิก</Button>
			<Button on:click={handleAssignRole} disabled={assigning || !selectedRoleId}>
