...
export interface AdmissionRound {
	...
	selectionSettings?: {
		subjectsByTrack?: Record<string, string[]>;
		methodByTrack?: Record<string, string>;
		method: string;
		assignmentMode?: 'per_track' | 'global';
		showScores?: boolean;
	};
}
...
export const DOC_TYPE_LABELS: Record<string, { label: string; required: boolean }> = {
	...
	birth_cert: {
		label: 'สำเนาสูติบัตร (กรณีไม่มีบัตรประชาชนและทะเบียนบ้านบิดา/มารดา)',
		required: false
	}
};
...
export async function getTrackCapacity(id: string) {
	const res = await apiClient.get<{
		rooms: { roomId: string; roomName: string; roomCode: string }[];
		roomCount: number;
	}>(`/api/admission/tracks/${id}/capacity`);
	if (!res.success) throw new Error(res.error);
	return res.data!;
}
...
export async function createSubject(
	...
	const res = await apiClient.post<AdmissionExamSubject>(
		`/api/admission/rounds/${roundId}/subjects`,
		data
	);
	if (!res.success) throw new Error(res.error);
...
export async function getPublicRoundInfo(roundId: string) {
	const res = await apiClient.get<{ round: AdmissionRound; tracks: AdmissionTrack[] }>(
		`/api/admission/apply/round/${roundId}`
	);
...
export async function submitApplication(roundId: string, data: SubmitApplicationData) {
	const res = await apiClient.post<{ applicationNumber: string }>(
		`/api/admission/apply/${roundId}`,
		data
	);
