...
test('assessment route requires an explicit term context', async () => {
	...
	assert.match(meta, /url\.searchParams\.get\('academicTermId'\)/);
	assert.match(meta, /listAssessmentPlans\(\{ academicTermId \}, \{ requestFetch: fetch \}\)/);
	assert.match(meta, /listAssessmentPhaseControls\(academicTermId, \{ requestFetch: fetch \}\)/);
	assert.match(page, /const academicTermId = \$derived\(data\.academicTermId\)/);
	assert.doesNotMatch(page, /academicContext\.subscribe|loadWorkspace/);
	const mountBody = page.slice(page.indexOf('onMount(() => {'), page.indexOf('$effect.pre'));
	assert.doesNotMatch(mountBody, /listAssessmentPlans|listAssessmentPhaseControls/);
	assert.match(meta, /captureRouteLoad/);
	assert.match(page, /plansLoading && !plansLoaded/);
	assert.match(page, /controlsLoading && !controlsLoaded/);
	assert.match(page, /patchPlanSummary\(saved\)/);
	assert.doesNotMatch(page, /refreshPlans\(/);
