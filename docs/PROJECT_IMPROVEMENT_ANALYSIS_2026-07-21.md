...
อัปเดตสถานะ 21 กรกฎาคม 2026: ช่องโหว่ที่บันทึกในหัวข้อนี้แก้แล้ว โดย handshake ตรวจ token และ tenant ก่อน upgrade, identity/room มาจาก server, service ตรวจสิทธิ์อ่าน/จัดการและ semester, server กรอง event ตามสิทธิ์, จำกัด frame 64 KiB, มี heartbeat/idle timeout และ frontend reconnect แบบ bounded พร้อมยกเลิก timer/listener เมื่อ teardown ข้อความด้านล่างเก็บไว้เป็นหลักฐานของสภาพก่อนแก้ ไม่ใช่พฤติกรรมปัจจุบัน
...
- PASS — `cd backend-school && cargo test --bin backend-school -- --skip modules::auth::tests::auth_tests::test_login_success --skip modules::auth::tests::auth_tests::test_login_invalid_credentials` (exit 0; 416 passed, 0 failed, 2 filtered out).
- PASS — `cd backend-school && cargo test --test static_architecture` (exit 0; 85 current static guards passed).
...
- PASS — `cd frontend-school && npm run test:static` (exit 0; 252 passed, 0 failed).
...
- PASS — scoped frontend query-contract search over `TimetableSocketParams` and the `URLSearchParams` construction found no `school_key`, query `name`, or query `user_id` (`rg` exit 1 as expected).
...
- PASS — `git diff --check` (exit 0 before recording this result).
- PASS — `git diff --name-only -- backend-school/migrations` returned no migration paths.
- PASS — `git diff --name-only 9095cc1e..edeb126e -- backend-school/migrations` returned no migration paths across the implementation.
