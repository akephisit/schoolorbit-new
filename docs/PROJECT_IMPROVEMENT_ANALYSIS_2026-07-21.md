...
|---|---|---|
| P0 | WebSocket timetable ไม่ตรวจ JWT และรับ `user_id`, `name`, `school_key` จาก query | ตรวจ JWT ก่อน upgrade, resolve tenant ด้วย resolver กลาง, derive user จาก claims และตรวจ permission/semester |
| P0 | JWT ไม่ผูกกับ tenant | เพิ่ม `tenant_id` หรือ `subdomain`, `iss`, `aud` และ session version ใน claims |
| P0 | Permission cache ใช้เฉพาะ `user_id` เป็น key | เปลี่ยนเป็น `(tenant_id, user_id)` และเพิ่ม cross-tenant negative tests |
...
- WebSocket authentication และ authorization
- tenant-bound JWT และ permission cache key
